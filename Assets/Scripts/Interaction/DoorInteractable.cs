...
*  la puerta puede abrirse o no, si necesita una condici�n del juego o no y qu� mensaje mostrar cuando est� bloqueada
...
public class DoorInteractable : Interactable
{
    ...
    public override void Interact(PlayerInteraction player)
    {
        ...
        // si necesita una condici�n y no se cumple mete feedback y sale
        if (!CanOpenByState())
        {
            SubtitleUI.Instance.ShowSubtitle(GetLockedFeedbackMessage(), 2.5f);
            if (ClosedSound) SFXManager.Instance.Play3D("LockedDoor", transform.position);
